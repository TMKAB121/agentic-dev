...
process.stdin.on('end', () => {
  try {
    ...
    // Tolerate plugin namespacing — an installed plugin may surface an agent
    // as "agentic-dev:qa-engineer".
    const agentType =
      input.agent_type && input.agent_type.includes(':')
        ? input.agent_type.split(':').pop()
        : input.agent_type;
    if (agentType && !PIPELINE_AGENTS.has(agentType)) process.exit(0);
